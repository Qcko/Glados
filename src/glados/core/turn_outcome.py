...
Caveat: pure error-derivation misses *semantic* drift, where a tool returns
200 OK but the model never acts on the result (bake-off T2: ``search_products``
succeeded, the model just never added to the cart). A later slice adds a
lightweight goal-check; this module deliberately stays in the deterministic
